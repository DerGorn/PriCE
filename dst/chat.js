...
const onCallClickHandler = async () => {
    if (audioCall) {
        if (callIcon == null) {
            throw new Error("Clicked on a non existend Button. 'callIcon' was clicked but is 'null'.");
        }
...
const onStreamClickHandler = async () => {
    if (videoCall) {
        if (streamIcon == null) {
            throw new Error("Clicked on a non existend Button. 'streamIcon' was clicked but is 'null'.");
        }
